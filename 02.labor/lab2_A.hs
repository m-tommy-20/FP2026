import Text.Parsec.Token (GenLanguageDef(reservedNames), GenTokenParser (natural))
-- # 2. labor
...
szjSzamOsszeg2 n szj elof
  | szj > 9 = error "nem szj"
  | n < 10 = if n == szj then (elof + 1) * szj else szj * elof
...
parosSzjSzam n res
  | n < 0 = parosSzjSzam (abs n) res
  | n < 10 = if even n then res + 1 else res
  | otherwise =
    if even (mod n 10)
      then parosSzjSzam (div n 10) (res + 1)
      else parosSzjSzam (div n 10) res
  
parosSzjSzam2 n
  | n < 0 = parosSzjSzam2 (abs n)
  | n < 10 = if even n then 1 else 0
  | otherwise =
    if even (mod n 10)
      then parosSzjSzam2 (div n 10) + 1
      else parosSzjSzam2 (div n 10)
...
bSzamrDSzjLs = map(\(n,b,d) -> bSzamDSzj n b d) ls3
